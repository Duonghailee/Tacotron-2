...
def log_prob_from_logits(x):
	...
	axis = len(x.get_shape())-1
	m = tf.reduce_max(input_tensor=x, axis=axis, keepdims=True)
	return x - m - tf.math.log(tf.reduce_sum(input_tensor=tf.exp(x-m), axis=axis, keepdims=True))
...
def discretized_mix_logistic_loss(y_hat, y, num_classes=256,
	...
	Returns:
		Tensor loss
	'''
	with tf.control_dependencies([tf.compat.v1.assert_equal(tf.math.floormod(tf.shape(input=y_hat)[1], 3), 0), tf.compat.v1.assert_equal(tf.rank(y_hat), 3)]):
	...
	log_probs = tf.compat.v1.where(y < -0.999, log_cdf_plus,
		tf.compat.v1.where(y > 0.999, log_one_minus_cdf_min,
			tf.compat.v1.where(cdf_delta > 1e-5,
...
def sample_from_discretized_mix_logistic(y, log_scale_min=-7.):
	...
	'''
	with tf.control_dependencies([tf.compat.v1.assert_equal(tf.math.floormod(tf.shape(input=y)[1], 3), 0)]):
	...
	temp = tf.random.uniform(tf.shape(input=logit_probs), minval=1e-5, maxval=1. - 1e-5)
	...
	u = tf.random.uniform(tf.shape(input=means), minval=1e-5, maxval=1. - 1e-5)
